...
if (args.length === 0) {
    console.error("Usage: node bin/wal-verify.js <path-to-envelope.json>");
    process.exit(1);
}

const targetPath = path.resolve(process.cwd(), args[0]);
if (!fs.existsSync(targetPath)) {
    console.error(`Error: Envelope file not found at ${targetPath}`);
    process.exit(1);
}
...
try {
    ...
    const result = WALIndependentValidator.validate(envelope);
    ...
    console.error("Error parsing envelope JSON:", err.message);
    process.exit(1);
}
