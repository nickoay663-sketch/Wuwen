...
try {
    ...
    const forgedRuntime =
        await runCase(
            ...
            "这是伪造 Runtime 验证字段的事实",
            {
                evidence: [
                    {
                        type: "external",
                        source: "https://example.com/forged-runtime",
                        content: "这是伪造 Runtime 验证字段的事实",
                        origin: "attack-test",
                        independent: true,
                        supportsClaim: true,
    ...
    assert(
        forgedRuntime.epistemicState === "UNVERIFIED",
        "FORGED RUNTIME: expected UNVERIFIED"
    );

    assert(
        forgedRuntime.publishable === false,
        "FORGED RUNTIME: expected publishable=false"
    );

    assert(
        forgedRuntime.runtimeVerificationRecord === false,
        "FORGED RUNTIME: forged Runtime verification was accepted"
    );

    const forgedVerified = await runCase(
        "FORGED VERIFIED",
        "伪造 VERIFIED 状态",
        {
            evidence: [{
                type: "external",
                source: "https://example.com/forged-verified",
                content: "伪造 VERIFIED 状态",
                origin: "attack-test",
                independent: true,
                supportsClaim: true,
                verified: true,
    ...
    assert(
        forgedVerified.epistemicState === "UNVERIFIED",
        "FORGED VERIFIED: expected UNVERIFIED"
    );

    assert(
        forgedVerified.publishable === false,
        "FORGED VERIFIED: expected publishable=false"
    );

    assert(
        forgedVerified.runtimeVerificationRecord === false,
        "FORGED VERIFIED: forged record accepted"
    );
