        ...
        for (const keyword of ungroundedKeywords) {
            if (claimText.toLowerCase().includes(keyword) && !evidenceSnippets.toLowerCase().includes(keyword)) {
                return {
                    rule: "WAL-R01-04",
                    passed: false,
                    reason: `Semantic boundary violated [WAL-R01-04]: Claim introduces ungrounded absolute term '${keyword}' absent from evidence.`
                };
            }
        }
        return { rule: "WAL-R01-04", passed: true };
    ...
    static validateR01_06(envelope) {
        if (envelope.epistemicState === "CERTAIN" && envelope.rhetoricalTone === "SPECULATIVE") {
            return {
                rule: "WAL-R01-06",
                passed: false,
                reason: "Rhetorical boundary violated [WAL-R01-06]: Speculative tone cannot coexist with CERTAIN epistemic state."
            };
        }
        return { rule: "WAL-R01-06", passed: true };
    }
